...
##################################################################################
##                                                                              ##
##   this is the original CBC.pm from the Crypt::CBC module                     ##
##   for perl being little bit modified because md5!!!, yeah man                ##
##   it was still using md5 for passphrase based key derivation                 ##
##   now that everyone knows md5 is too fucked up on collision issues           ##
##   we think it's a simple example of excess of faggotism n'                   ##
##   too much NSA-manufucktured fishy fishy thingie                             ##
##   not doing anything about it                                                ##
##                                                                              ##
##   but, but...                                                                ##
##   WHAT THE FUCK THE 'fuckNSA' OPTION DOES?????                               ##
##                                                                              ##
##   well it just modifies 2 things about how a key and the iv is derivated     ##
##   from a passphrase when using the 'salt' option (that indeed will also      ##
##   break the pseudo-OpenSSL compatibility since OpenSSL currently defaults    ##
##   to md5 and 8 byte long salts. thought even when it would be possible to    ##
##   modify the digest algorithm using the undocumented flag -md ...            ##
##   example:                                                                   ##
##                                                                              ##
...
##   function (already implemented into the 'fuckNSA.pl' irssi perl script      ##
...
# < kayla> your grammar sucks
# < sup_g> did you call GCHQ teletubbies?

use constant RANDOM_DEVICE => '/dev/urandom';
##################################################################################
## well, check 'cat /dev/random ' and if you feel the result                    ##
## satisfy you then switch between options,                                     ##
## commenting the line of code above and uncommenting                           ##
## the line of code bellow                                                      ##
## otherwise try to improve the available entropy BEING FED to your system,     ##
## research by yourself, there're many solutions out there,                     ##
## including dedicated hardware devices and shits.                              ##
## Remember having good randomness available from your system is for your       ##
## security as important as having money when you go out with hookers.          ##
## anyway '/dev/random ' must be enabled for BSD systems...                     ##
##################################################################################

#use constant RANDOM_DEVICE => '/dev/random';
# ^ this line
...
sub new {
    ...
    my $fuckNSA = 0;
    $fuckNSA = $options->{'fuckNSA'} if exists $options->{'fuckNSA'};
    ## ^ ADDED THIS TO MAKE AVAILABLE THE OPTION TO ENFORCE PASSPHRASE KEY DERIVATION ##
...
sub _generate_iv_and_cipher_from_options {
  ...
  if ($header_mode eq 'none') {
	...
	if ($self->{fuckNSA} == 1) {  
		$self->{salt} = $self->_get_random_bytes(16) if $self->{make_random_salt};
	    defined (my $salt = $self->{salt}) or croak "No header_mode of 'salt' specified, but no salt value provided"; # shouldn't happen
	    length($salt) == 16 or croak "Salt must be exactly 16 bytes long";
	    my ($key,$iv) = $self->_salted_key_and_iv($self->{passphrase},$salt);
	    $self->{key}  = $key;
	    $self->{civ}  = $self->{iv} = $iv;
...
sub _key_from_key {
 ...
 # my $material = md5($pass);
  ...
  if ($self->{fuckNSA} == 1) {
	  $material = sha512($pass);
	    while (length($material) < $ks)  {
            $material .= sha512($material);
        }
  }
...
sub _salted_key_and_iv {
  ...
  if ($self->{fuckNSA} == 1) {
   croak "Salt must be 16 bytes long" unless length $salt == 16;
  }else{
